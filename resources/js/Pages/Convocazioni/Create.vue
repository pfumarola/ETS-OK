...
const form = useForm({
    tipo: props.tipoOptions?.[0]?.value ?? 'assemblea',
    titolo: '',
    scheduled_at: props.defaultValues?.scheduled_at ?? '',
    luogo: '',
    ordine_del_giorno: '',
});
</script>
...
<template>
    <AppLayout title="Nuova convocazione">
        <Head title="Nuova convocazione" />
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Nuova convocazione</h2>
        ...
        <div class="py-6 max-w-4xl mx-auto sm:px-6">
            <form @submit.prevent="form.post(route('convocazioni.store'))" class="space-y-4 bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <div>
                ...
                <div class="pt-2">
                    <PrimaryButton :disabled="form.processing">Salva bozza</PrimaryButton>
                </div>
            </form>
        </div>
    </AppLayout>
