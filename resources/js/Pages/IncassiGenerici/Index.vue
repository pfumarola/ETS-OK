...
const search = () => router.get(route('incassi-generici.index'), form);
</script>

<template>
    <AppLayout title="Incassi generici">
        <Head title="Incassi generici" />
        <template #header>
            <div class="flex flex-wrap justify-between items-center gap-2">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Incassi generici</h2>
                <Link :href="route('incassi.create', { type: 'altro' })">
                    <PrimaryButton><PlusIcon class="size-4 me-2" aria-hidden="true" />Nuovo incasso generico</PrimaryButton>
                </Link>
            </div>
        </template>
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Destinatario</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Causale</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="i in incassi.data" :key="i.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-2">{{ i.member ? i.member.cognome + ' ' + i.member.nome : (i.donor_name || '—') }}</td>
                    ...
                    <p v-if="!incassi.data?.length" class="px-4 py-8 text-center text-gray-500">Nessun incasso generico.</p>
