...
const props = defineProps({ memberType: Object });

const form = useForm({
    name: props.memberType.name,
    display_name: props.memberType.display_name ?? props.memberType.name,
});
</script>

<template>
    <AppLayout title="Modifica tipologia socio">
        <Head title="Modifica tipologia socio" />
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Modifica tipologia</h2>
        ...
        <div class="py-6 max-w-2xl mx-auto sm:px-6">
            <form @submit.prevent="form.put(route('member-types.update', memberType.id))" class="space-y-4 bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <div>
                    <InputLabel for="name" value="Codice (nome interno) *" />
                    <TextInput id="name" v-model="form.name" class="mt-1 block w-full" maxlength="50" />
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Solo lettere minuscole, numeri e trattini.</p>
                    <InputError class="mt-1" :message="form.errors.name" />
                </div>
                <div>
                    <InputLabel for="display_name" value="Nome visualizzato *" />
                    <TextInput id="display_name" v-model="form.display_name" class="mt-1 block w-full" maxlength="100" />
